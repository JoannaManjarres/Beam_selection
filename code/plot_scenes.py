...
def plot_3D_scene_with_rx_as_cube(data_lidar_process_all, data_position_rx, data_position_tx, data_position_rx_as_cube,sample_for_plot):
    print ("plotando o cenario completo em 3D com o Rx em forma de Termometro ...")
    # ------- PLOT RX E CENA COMPLETA
    sample_for_plot = sample_for_plot
    rx_as_cube = data_position_rx_as_cube [sample_for_plot, :, :, :]
    rx = data_position_rx [sample_for_plot, :, :, :]
    tx = data_position_tx [sample_for_plot, :, :, :]
    scenario_complet = data_lidar_process_all [sample_for_plot, :, :, :]
    ...
    title = 'Scene ' + str (sample_for_plot)
    plt.title (title, fontsize=20)

    ax = fig.add_subplot (1, 2, 1, projection='3d')
    ax.voxels (rx_as_cube, alpha=0.12, edgecolor=None, shade=True, color='red')  # Voxel visualization
    ax.set_title ('Rx as Termomether', fontsize=14)
    ax.set_xlabel ('x', labelpad=10)
    ax.set_ylabel ('y', labelpad=10)
    ax.set_zlabel ('z', labelpad=10)
    plt.tight_layout()
    ...
    #         color='#cccccc90')  # Voxel visualization
    ax.voxels(voxelarray, facecolors=colors, edgecolor=None, antialiased=False)
    ax.set_title('Full scenario', fontsize=10)
    ax.set_xlabel('x', labelpad=10)
    ax.set_ylabel('y', labelpad=10)
    ax.set_zlabel('z', labelpad=10)
    plt.tight_layout()
    ...
    ax.legend(handles=[c1, c2, c3], loc='upper right', bbox_to_anchor=(-0.1, 0.9)) #
